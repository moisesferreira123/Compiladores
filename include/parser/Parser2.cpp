...
void Stmt() {
   ...
   if (lookahead == NAME) {
     ...
     || lookahead == TOKEN_NEW || lookahead == TOKEN_NULL) {
      ...
      std::cerr << "Erro sintático na linha " << numLines << ", coluna "
                << numCols << ". Esperado token " << NAME << " ou "
                << TOKEN_DEREF << " ou " << TOKEN_IF << " ou " << TOKEN_WHILE
                << " ou " << TOKEN_RETURN << " mas encontrado "
                << scanner.YYText() << "\n";
      exit(1);
   }
}
...
void Type() {
   if (lookahead == NAME) {
      ...
      std::cerr << "Erro sintático na linha " << numLines << ", coluna "
                << numCols << ". Esperado token " << NAME << " ou "
                << TOKEN_FLOAT << " ou " << TOKEN_INT << " ou " << TOKEN_STRING
                << " ou " << TOKEN_BOOL << " ou " << TOKEN_REF
                << " mas encontrado " << scanner.YYText() << "\n";
      exit(1);
   }
}
...
void Fact() {
   if (lookahead == TOKEN_OPEN_PARENTHESIS) {
     ...
     || lookahead == TOKEN_FALSE || lookahead == TOKEN_NULL) {
      ...
      std::cerr << "Erro sintático na linha " << numLines << ", coluna "
                << numCols << ". Esperado token " << TOKEN_OPEN_PARENTHESIS
                << " ou " << NAME << " ou " << TOKEN_REF << " ou "
                << TOKEN_DEREF << " ou " << FLOAT_LITERAL << " ou "
                << INT_LITERAL << " ou " << STRING_LITERAL << " ou "
                << TOKEN_TRUE << " ou " << TOKEN_FALSE << " ou " << TOKEN_NULL
                << " ou " << TOKEN_NEW << " mas encontrado " << scanner.YYText()
                << "\n";
      exit(1);
   }
}
...
void Literal() {
   if (lookahead == FLOAT_LITERAL) {
      ...
      std::cerr << "Erro sintático na linha " << numLines << ", coluna "
                << numCols << ". Esperado token " << FLOAT_LITERAL << " ou "
                << INT_LITERAL << " ou " << STRING_LITERAL << " ou "
                << TOKEN_TRUE << " ou " << TOKEN_FALSE << " ou " << TOKEN_NULL
                << " mas encontrado " << scanner.YYText() << "\n";
      exit(1);
...
void BoolLiteral() {
   if (lookahead == TOKEN_TRUE) {
      ...
      std::cerr << "Erro sintático na linha " << numLines << ", coluna "
                << numCols << ". Esperado token " << TOKEN_TRUE << " ou "
                << TOKEN_FALSE << " mas encontrado " << scanner.YYText()
                << "\n";
      exit(1);
   }
}
...
void Var2() {
   if (lookahead == NAME) {
     ...
     || lookahead == TOKEN_NEW) {
      ...
      std::cerr << "Erro sintático na linha " << numLines << ", coluna "
                << numCols << ". Esperado token " << TOKEN_NOT << " ou "
                << TOKEN_OPEN_PARENTHESIS << " ou " << TOKEN_REF << " ou "
                << TOKEN_DEREF << " ou " << FLOAT_LITERAL << " ou "
                << INT_LITERAL << " ou " << STRING_LITERAL << " ou "
                << TOKEN_TRUE << " ou " << TOKEN_FALSE << " ou " << TOKEN_NULL
                << " ou " << TOKEN_NEW << " mas encontrado " << scanner.YYText()
...
void Fact2() {
   if (lookahead == TOKEN_OPEN_PARENTHESIS) {
     ...
     || lookahead == TOKEN_FALSE || lookahead == TOKEN_NULL) {
      ...
      std::cerr << "Erro sintático na linha " << numLines << ", coluna "
                << numCols << ". Esperado token " << TOKEN_OPEN_PARENTHESIS
                << " ou " << TOKEN_REF << " ou " << TOKEN_DEREF << " ou "
                << FLOAT_LITERAL << " ou " << INT_LITERAL << " ou "
                << STRING_LITERAL << " ou " << TOKEN_TRUE << " ou "
                << TOKEN_FALSE << " ou " << TOKEN_NULL << " ou " << TOKEN_NEW
                << " mas encontrado " << scanner.YYText() << "\n";
      exit(1);
   }
}
...
void OrOp3() {
   if (lookahead == TOKEN_OR) {
     ...
     || lookahead == TOKEN_POT || lookahead == TOKEN_EQUAL) {
      ...
      std::cerr << "Erro sintático na linha " << numLines << ", coluna "
                << numCols << ". Esperado token " << TOKEN_OR << " ou "
                << TOKEN_AND << " ou " << TOKEN_COMP << " ou " << TOKEN_EQUAL
                << " ou " << TOKEN_ADD << " ou " << TOKEN_SUB << " ou "
                << TOKEN_DIV << " ou " << TOKEN_MULT << " ou " << TOKEN_POT
                << " mas encontrado " << scanner.YYText() << "\n";
      exit(1);
   }
}
...
void AndOp3() {
   if (lookahead == TOKEN_AND) {
     ...
     || lookahead == TOKEN_EQUAL) {
      ...
      std::cerr << "Erro sintático na linha " << numLines << ", coluna "
                << numCols << ". Esperado token " << TOKEN_AND << " ou "
                << TOKEN_COMP << " ou " << TOKEN_EQUAL << " ou " << TOKEN_ADD
                << " ou " << TOKEN_SUB << " ou " << TOKEN_DIV << " ou "
...
void ExpRel3() {
   if (lookahead == TOKEN_COMP) {
     ...
     || lookahead == TOKEN_POT) {
      ...
      std::cerr << "Erro sintático na linha " << numLines << ", coluna "
                << numCols << ". Esperado token " << TOKEN_COMP << " ou "
                << TOKEN_EQUAL << " ou " << TOKEN_ADD << " ou " << TOKEN_SUB
                << " ou " << TOKEN_DIV << " ou " << TOKEN_MULT << " ou "
                << TOKEN_POT << " mas encontrado " << scanner.YYText() << "\n";
      exit(1);
   }
}
...
void ExpArith3() {
   if (lookahead == TOKEN_ADD) {
     ...
     || lookahead == TOKEN_POT) {
      ...
      std::cerr << "Erro sintático na linha " << numLines << ", coluna "
                << numCols << ". Esperado token " << TOKEN_ADD << " ou "
                << TOKEN_SUB << " ou " << TOKEN_DIV << " ou " << TOKEN_MULT
                << " ou " << TOKEN_POT << " mas encontrado " << scanner.YYText()
                << "\n";
      exit(1);
   }
}
...
void Stmt_() {
   if (lookahead == TOKEN_OPEN_PARENTHESIS) {
     ...
     || lookahead == TOKEN_EQUAL) {
      ...
      std::cerr << "Erro sintático na linha " << numLines << ", coluna "
                << numCols << ". Esperado token " << TOKEN_OPEN_PARENTHESIS
                << " ou " << TOKEN_OR << " ou " << TOKEN_AND << " ou "
                << TOKEN_COMP << " ou " << TOKEN_EQUAL << " ou " << TOKEN_ADD
                << " ou " << TOKEN_SUB << " ou " << TOKEN_DIV << " ou "
                << TOKEN_MULT << " ou " << TOKEN_POT << " ou " << TOKEN_DOT
                << " ou " << TOKEN_ATTRIBUTION << " mas encontrado "
                << scanner.YYText() << "\n";
      exit(1);
   }
}
...
void Fact5() {
   if (lookahead == TOKEN_OPEN_PARENTHESIS) {
      ...
      RefVar();
   } else if (lookahead == lookahead == FLOAT_LITERAL
     || lookahead == INT_LITERAL || lookahead == STRING_LITERAL
     || lookahead == TOKEN_TRUE || lookahead == TOKEN_FALSE
     || lookahead == TOKEN_NULL) {
      Literal();
   } else if (lookahead == TOKEN_NEW) {
      match(TOKEN_NEW);
      match(NAME);
      ...
      std::cerr << "Erro sintático na linha " << numLines << ", coluna "
                << numCols << ". Esperado token " << TOKEN_OPEN_PARENTHESIS
                << " ou " << TOKEN_REF << " ou " << FLOAT_LITERAL << " ou "
                << INT_LITERAL << " ou " << STRING_LITERAL << " ou "
                << TOKEN_TRUE << " ou " << TOKEN_FALSE << " ou " << TOKEN_NULL
                << " ou " << TOKEN_NEW << " mas encontrado " << scanner.YYText()
                << "\n";
