...
class CodeEmitter {
   ...
   // em um só passo.
   void emit(OpCode op, std::string result, std::string arg1 = "",
     std::string arg2 = "");
   ...
   void emitProcedureReturn();

   void emitCallStmt(const std::string& procedure_name,
     std::vector<TacOperand*>* call_args, std::string temp);

   bool emitDefaultCallStmt(const std::string& procedure_name,
     std::vector<TacOperand*>* call_args, std::string temp);
